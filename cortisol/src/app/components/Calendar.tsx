...
function fakeFetch(date: Dayjs, { signal }: { signal: AbortSignal }) {
    return new Promise<{ daysToHighlightGreen: number[] ; daysToHighlightPurple: number[]}>((resolve, reject) => {
...
export default function DateCalendarServerRequest() {
    ...
    const [highlightedDaysGreen, setHighlightedDaysGreen] = React.useState([1, 2, 15]);
    const [highlightedDaysPurple, setHighlightedDaysPurple] = React.useState([2, 7, 23])
    ...
    return (
        ...
        <DateCalendar
            ...
            slots={{
                day: (props: PickersDayProps<Dayjs> & { highlightedDaysGreen?: number[]; highlightedDaysPurple?: number[] }) => {
                    const { highlightedDaysGreen = [], highlightedDaysPurple = [], outsideCurrentMonth, day, ...other } = props;
                    const isGreenSelected = highlightedDaysGreen.includes(props.day.date()) && !props.outsideCurrentMonth;
                    const isPurpleSelected = highlightedDaysPurple.includes(props.day.date()) && !props.outsideCurrentMonth;
